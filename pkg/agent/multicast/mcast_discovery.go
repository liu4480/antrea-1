...
func (s *IGMPSnooper) validate(event *mcastGroupEvent) (bool, error) {
	if !s.anpEnabled {
		// Validator is null if anpEnabled is false, thus here returns directly
		return true, nil
	}
...
func newSnooper(ofClient openflow.Client, ifaceStore interfacestore.InterfaceStore, eventCh chan *mcastGroupEvent, queryInterval time.Duration, Validator types.MulticastValidator, anpEnabled bool) *IGMPSnooper {
	d := &IGMPSnooper{ofClient: ofClient, ifaceStore: ifaceStore, eventCh: eventCh, queryInterval: queryInterval, Validator: Validator, anpEnabled: anpEnabled}
