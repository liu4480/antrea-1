...
// Another issue, is that multicast stats will also count IGMP v1 report packet into normal multicast
// egress rule as well.
func (f *featureMulticast) igmpEgressFlow() []binding.Flow {
	flows := []binding.Flow{
		MulticastEgressRuleTable.ofTable.BuildFlow(priorityTopAntreaPolicy).
			Cookie(f.cookieAllocator.Request(f.category).Raw()).
			MatchProtocol(binding.ProtocolIGMP).
			MatchRegMark(FromLocalRegMark).
			Action().GotoTable(MulticastRoutingTable.GetID()).
			Done(),
	}
	return flows
}
