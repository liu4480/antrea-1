...
type Client interface {
	...
	InstallMulticastGroup(ofGroupID binding.GroupIDType, isIGMPQueryGroup bool, localReceivers []uint32) error
}
...
func (c *client) InstallMulticastGroup(groupID binding.GroupIDType, isIGMPQueryGroup bool, localReceivers []uint32) error {
	c.replayMutex.RLock()
	defer c.replayMutex.RUnlock()
	...
	targetPorts := append([]uint32{}, localReceivers...)
	if !isIGMPQueryGroup {
		targetPorts = append(targetPorts, config.HostGatewayOFPort)
	}

	if err := c.featureMulticast.multicastReceiversGroup(groupID, table.GetID(), targetPorts...); err != nil {
		return err
	}
	return nil
}
